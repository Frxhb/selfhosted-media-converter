...
async function checkAndUpdateYtDlp() {
  ...
  try {
    ...
    const displayEl = document.getElementById("ytdlp-version-display");
    if (displayEl) {
      displayEl.textContent = data.version || t("label.unknown");
      
      if (data.url) {
        displayEl.href = data.url;
      } else {
    ...
    if (data.updated) {
      ...
      showToast(
        t("toast.ytdlp_already_latest").replace("{version}", data.version || "").trim(),
        "info"
      );
    }
